	    	 ...
	    	 <form id="listForm" action="list" method="get">
						...
						<div id="searchBar" class="input-group m-b">
							<div class="input-group-btn">
		                        ...
		                        <ul id="searchPropertyOption" class="dropdown-menu">
									<li>
										<a href="javascript:;" val="username"  [#if page.searchProperty == "username"]selected[/#if]>用户名</a>
									</li>
									<li>
										<a href="javascript:;" val="department"  [#if page.searchProperty == "department"]selected[/#if]>部门</a>
									</li>
									<li>
										<a href="javascript:;" val="name"  [#if page.searchProperty == "name"]selected[/#if]>姓名</a>
									</li>
								</ul>
							</div>
				...
				<div class="table-responsive">
					<table id="listTable" class="table table-bordered table-striped table-hover">
						...
						<tr>
							<td><input type="checkbox" name="ids"[#if admin.id=1] disabled="disabled"[/#if] value="${admin.id}" /></td>
				 			<td>${admin.username}</td>	
				 			<td>${admin.email}</td>
				 			<td>${admin.name}</td>
				 			<td>${admin.department}</td>
							<td>
								<a href="${base}/admin/edit?id=${admin.id}" class="btn btn-sm btn-white btn-bitbucket">
